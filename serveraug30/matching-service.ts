 ...
 * 
 * Provides AI-powered matching between users and projects using OpenAI embeddings.
 * Generates semantic embeddings for user profiles and projects, then calculates
 * similarity scores to find relevant matches.
 * 
 * Main Class:
 * - MatchingService: AI-powered matching service
 *   - generateUserEmbedding(user): Creates semantic embedding for user profile
 *   - generateProjectEmbedding(project): Creates semantic embedding for project
 *   - calculateCosineSimilarity(vecA, vecB): Calculates vector similarity (0-1 scale)
 *   - findMatches(user): Finds projects matching a user profile
 *   - findMatchingUsers(project): Finds users matching a project
 *   - rankMatches(...): Ranks matches by similarity score
 * 
 * Private Helper Methods:
 * - createUserProfileText(user): Converts user data to text for embedding
 * - createProjectText(project): Converts project data to text for embedding
 * - ensureOpenAI(): Validates OpenAI API is configured
 * - openAIEnabled(): Checks if OpenAI is available
 * 
 * Features:
 * - Semantic similarity using OpenAI embeddings
 * - Considers user skills, interests, role, and affiliation
 * - Considers project description, required skills, and research area
 * - Graceful degradation when OpenAI not configured
 * 
 * Environment Variables Required:
 * - OPENAI_API_KEY: OpenAI API key (optional, disables matching if not set)
 */
