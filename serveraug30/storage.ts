 ...
 * 
 * Defines the IStorage interface that abstracts all data persistence operations.
 * This interface is implemented by storage providers (Firebase, PostgreSQL, etc.)
 * allowing the application to work with different storage backends.
 * 
 * Main Interface:
 * - IStorage: Defines all storage operation contracts
 *   - User operations: CRUD for users, username/email lookups
 *   - Project operations: CRUD for projects with filtering
 *   - Opportunity operations: Opportunity management and matching
 *   - Application operations: Application submission and tracking
 *   - Message operations: Conversation and messaging
 *   - Grant operations: Research grant management
 *   - Notification operations: User notifications
 *   - Project Chat: Group chat functionality
 *   - Feed & Activity: Social feed and activity tracking
 *   - LiveEvent operations: Event creation and registration
 *   - Interest matching: User interests and recommendations
 * 
 * Data Type Interfaces:
 * - Activity: User activity record (id, message, actorId, createdAt)
 * - LiveEvent: Event record with registration tracking
 * - InsertActivity, InsertLiveEvent: Types for creating new records
 * 
 * Design Pattern:
 * - This follows the Repository pattern
 * - Allows implementation switching without changing application code
 * - Type-safe with TypeScript generics
 * - Supports both relational and NoSQL backends
 */
