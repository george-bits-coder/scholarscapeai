 ...
 * 
 * Implements the IStorage interface using Firebase Realtime Database.
 * Provides all CRUD operations for managing users, projects, opportunities,
 * applications, messages, grants, notifications, and other domain entities.
 * 
 * Main Class:
 * - FirebaseStorage: Complete implementation of IStorage interface
 *   - User operations: getUser, createUser, updateUser, getUserByUsername, getUserByEmail
 *   - Project operations: getProject, getProjects, createProject, updateProject, deleteProject
 *   - Opportunity operations: similar CRUD methods
 *   - Application management: getApplicationsForProject, createApplication, etc.
 *   - Messaging: getMessages, createMessage, deleteMessage
 *   - Live Events: getLiveEvent, getLiveEvents, createLiveEvent, etc.
 *   - Activity tracking: getRecentActivities, createActivity, getFeed methods
 *   - Notifications: createNotification, getNotifications, markAsRead
 *   - Project Chats: Project chat and member management
 *   - User Interests: Interest tracking and matching
 * 
 * Main Interfaces:
 * - IStorage: Defines all storage operation contracts
 * - LiveEvent: Event data structure with registration tracking
 * - Activity: User activity for feeds
 * - Various Insert* types for creating new records
 * 
 * Storage Patterns:
 * - All data persisted to Firebase Realtime Database
 * - Uses consistent path naming conventions (e.g., 'users/userId')
